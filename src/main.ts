import { isMerchant, merchantCapacity, nearestCenter, LOCAL_RADIUS } from './logistics.ts';
...
import { knownRegions, outpostsUnlocked, regionInfo, BIOMES, ERAS, populationCap, civilisationProgress, advanceCivilisation, expeditionStatus, explore, worldBounds, spendableStock, workerTarget, productionChoice, productionChoices, setProductionRecipe } from './sim.ts';
...
if (import.meta.env.DEV && sandbox) {
  ...
  if (scenario && ['village', 'world', 'mining', 'economy', 'surface', 'workers', 'trade'].includes(scenario)) {
...
el('app').innerHTML = `
  ...
  <div class="camera-hint">Linksziehen <span>Bewegen</span> <b>·</b> Rechtsziehen <span>Drehen</span> <b>·</b> Scrollen <span>Zoom</span> <b>·</b> WASD <span>Bewegen</span></div>
...
function setTool(next: Tool | null) {
  if (next === 'outpost' && !outpostsUnlocked(state)) { toast('Starte zuerst eine Expedition. Danach werden Außenposten freigeschaltet.'); return; }
...
function updateInspector() {
  ...
  const key = tool ? `tool-${tool}` : b ? `building-${b.id}-${b.complete}` : t ? `tile-${t.x}-${t.z}-${t.node}-${!!t.excavation}` : `intro-${state.level}-${state.regions.length}-${state.milestones.join(',')}`;
  if (key !== inspectorKey) {
    inspectorKey = key;
    ...
    if (tool) {
      ...
      if (b.complete && productionChoices(b).length) {
        const choice = el<HTMLSelectElement>('recipe-select');
        choice.value = b.pendingRecipe ?? productionChoice(b);
        choice.onchange = () => {
          if (setProductionRecipe(state, b, choice.value as keyof typeof NAMES)) {
  ...
  if (b && !tool) {
    if (document.getElementById('recipe-select')) {
      el('recipe-status').textContent = b.pendingRecipe ? `${NAMES[b.pendingRecipe]} vorgemerkt. Der laufende Auftrag wird noch beendet; danach wird das Rezept gewechselt.` : `Eingestellt: ${NAMES[productionChoice(b)]}. Änderungen gelten nach dem laufenden Auftrag.`;
    }
    ...
    if (b.complete) {
      const center = nearestCenter(state, b), residents = state.villagers.filter(v => v.home === b.id);
      const locality = center ? `${DEFINITIONS[center.kind].name} ${center.x} / ${center.z}` : 'Ehemaliger Heimatort';
      el('building-details').innerHTML = (b.kind === 'warehouse'
        ? `<section class="logistics-info"><div class="inventory-label">HÄNDLER & TRANSPORTE</div><p><b>${residents.filter(isMerchant).length} / 4 Händler</b> · bis ${merchantCapacity(state)} Waren je Fahrt</p><p>${residents.map(v => `${escape(v.name)}: ${v.task?.kind === 'return' ? v.task.vehicle ? 'bringt das Fahrzeug zurück' : 'kehrt zum Lager zurück' : v.task ? `${v.task.vehicle === 'coach' ? 'Kutsche' : v.task.vehicle === 'cart' ? 'Karren' : 'zu Fuß'} · ${v.cargo ? 'liefert' : 'holt'} ${v.task.amount} ${NAMES[v.task.resource]}` : 'wartet auf Bedarf'}`).join('<br>')}</p><p>Fuhrpark: ${state.level >= 3 ? `1 Pferdekarren (${residents.some(v => v.task?.vehicle === 'cart') ? 'unterwegs' : 'frei'})` : 'Pferdekarren ab Viehzucht'} · ${state.level >= 4 ? `1 Kutsche (${residents.some(v => v.task?.vehicle === 'coach') ? 'unterwegs' : 'frei'})` : 'Kutsche ab Kleinstadt'}.</p><p>Versorgt Baustellen und Betriebe aller Orte. Danach werden Lagerbestände zwischen Orten ausgeglichen.</p></section>`
        : `<section class="logistics-info"><p>Heimatort: ${locality} · ${LOCAL_RADIUS} Felder je Richtung.${['camp', 'house', 'miningHouse'].includes(b.kind) ? ` ${residents.length} Bewohner wohnen hier.` : ''}</p></section>`) + el('building-details').innerHTML;
    }
...
type Mission = { kind: BuildingKind | 'expedition'; title: string; sub: string };
const missions: Mission[] = [
  ...
  { kind: 'expedition', title: 'Neue Horizonte', sub: 'Starte deine erste Expedition' },
  { kind: 'outpost', title: 'Ein neuer Ort', sub: 'Gründe nach der Expedition einen Außenposten' },
];
const missionDone = (m: Mission) => m.kind === 'expedition' ? outpostsUnlocked(state) : state.milestones.includes(m.kind);
const missionAction = (m: Mission) => m.kind === 'expedition' ? 'Expeditionen öffnen' : `${DEFINITIONS[m.kind].name} planen`;
function startMission(m: Mission) { if (m.kind === 'expedition') showDevelopment(); else setTool(m.kind); }
function updateMissions() {
  ...
  if (!state.won) {
    ...
    const current = missions.findIndex(m => !missionDone(m));
    el('mission-list').innerHTML = missions.map((m, i) => { const done = missionDone(m); return `<li class="${done ? 'done' : i === current ? 'current' : ''}"><span class="step-circle">${done ? icon('check') : String(i + 1).padStart(2, '0')}</span><div><strong>${m.title}</strong><span>${m.sub}</span></div></li>`; }).join('');
    el('mission-fill').style.width = `${missions.filter(m => missionDone(m)).length / missions.length * 100}%`;
    el('mission-next').innerHTML = `<button class="text-button" id="next-mission">${missionAction(missions[Math.max(0, current)])} ${icon('arrow')}</button>`;
    el('next-mission').onclick = () => startMission(missions[Math.max(0, current)]);
  } else {
...
function updateBuildTools() {
  ...
  const markup = choices.map((t, i) => { const tier = t === 'road' ? 1 : DEFINITIONS[t].tier ?? 1; const expeditionLocked = t === 'outpost' && !outpostsUnlocked(state); return `<button class="build-tool ${tool === t ? 'active' : ''} ${expeditionLocked || tier > state.level ? 'locked' : ''}" data-tool="${t}" aria-label="${toolName(t)} bauen" aria-pressed="${tool === t}" ${expeditionLocked ? 'disabled' : ''} title="${expeditionLocked ? 'Nach der ersten Expedition' : tier > state.level ? 'Ab Stufe ' + ERAS[tier - 1].name : toolName(t)}"><kbd>${expeditionLocked ? 'Exp.' : tier > state.level ? 'St. ' + tier : i + 1}</kbd>${icon(t)}<span>${toolName(t)}</span></button>`; }).join('');
...
function renderDevelopment(refresh: boolean) {
  ...
  const missingBasics: BuildingKind[] = ['woodcutter', 'sawmill', 'quarry', 'house', 'warehouse'];
